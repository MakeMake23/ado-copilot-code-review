...
async function run(): Promise<void> {
  try {
    ...
    if (authors) {
      ...
      if (!authorList.includes(currentAuthor)) {
        ...
        tl.setResult(
          ...
          "Skipped: PR author not in configured authors list.",
        );
        return;
      }
    ...
    if (useSystemAccessToken) {
      ...
      console.log(
        "Using System.AccessToken (OAuth) for Azure DevOps authentication.",
      );
      ...
      console.log(
        "Using Personal Access Token for Azure DevOps authentication.",
      );
    } else {
      tl.setResult(
        tl.TaskResult.Failed,
    ...
    if (collectionUri) {
      ...
      console.log(
        `Constructed collection URI from organization: ${resolvedCollectionUri}`,
      );
    } else {
      ...
      if (systemCollectionUri) {
        ...
        console.log(
          `Auto-detected collection URI from System.CollectionUri: ${resolvedCollectionUri}`,
        );
      }
    }
    ...
    if (!project) {
      tl.setResult(
        tl.TaskResult.Failed,
        "Project is required. Either provide it as an input or ensure System.TeamProject is available.",
      );
      return;
    }
    ...
    if (!pullRequestId) {
      tl.setResult(
        tl.TaskResult.Failed,
        "Pull Request ID is required. Either provide it as an input or run this task as part of a PR validation build.",
      );
      return;
    }
    ...
    const repoNameFile = path.join(workingDirectory, "Repository_Name.txt");
    if (fs.existsSync(repoNameFile)) {
      const discoveredRepo = fs.readFileSync(repoNameFile, "utf8").trim();
      if (discoveredRepo) {
        repository = discoveredRepo;
        process.env["REPOSITORY"] = repository;
        console.log(`Repository context updated to: ${repository}`);
      }
    }

    // Step 3: Fetch PR changes (iteration details)
    console.log("\n[Step 3/4] Fetching pull request changes...");
    const prChangesScript = path.join(
      scriptsDir,
      "Get-AzureDevOpsPRChanges.ps1",
    );
    const iterationDetailsOutput = path.join(
      workingDirectory,
    ...
    if (promptRaw) {
      ...
      if (!fileContent.trim()) {
        tl.setResult(
          tl.TaskResult.Failed,
          `Raw prompt file is empty: ${promptFileRaw}`,
        );
        return;
      }
      promptFilePath = path.join(workingDirectory, "_copilot_prompt.txt");
      ...
      if (fileContent.includes('"')) {
        tl.setResult(
          tl.TaskResult.Failed,
          `Custom prompts cannot include double quotes ("). Please remove any double quotes from the prompt file: ${promptFile}`,
        );
        return;
      }
    ...
    if (customPromptText) {
      ...
      const mergedPrompt = templateContent.replace(
        "%CUSTOMPROMPT%",
        customPromptText,
      );
      console.log("\nCUSTOM PROMPT:\n" + mergedPrompt + "\n\n");
    ...
    const addCommentScriptSource = path.join(
      scriptsDir,
    ...
    const deleteCommentScriptSource = path.join(
      scriptsDir,
      "Delete-CopilotComment.ps1",
    );
    const addCommentScriptDest = path.join(
      workingDirectory,
      "Add-AzureDevOpsPRComment.ps1",
    );
    const commentScriptDest = path.join(
      workingDirectory,
    ...
    const deleteCommentScriptDest = path.join(
      workingDirectory,
      "Delete-CopilotComment.ps1",
    );
    fs.copyFileSync(addCommentScriptSource, addCommentScriptDest);
    console.log(
      `Copied Add-AzureDevOpsPRComment.ps1 to: ${addCommentScriptDest}`,
    );
    ...
    console.log(
      `Copied Delete-CopilotComment.ps1 to: ${deleteCommentScriptDest}`,
    );
...
async function installCopilotCli(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    if (isWindows()) {
      ...
      console.log(
        "Installing GitHub Copilot CLI via official install script...",
      );
    ...
    installProcess.on("close", (code: number | null) => {
      if (code === 0) {
        ...
        reject(
          new Error(`Failed to install GitHub Copilot CLI. Exit code: ${code}`),
        );
      }
    });
...
async function runCopilotCli(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      reject(
        new Error(
          `Copilot review timed out after ${timeoutMs / 60000} minutes`,
        ),
      );
    }, timeoutMs);
