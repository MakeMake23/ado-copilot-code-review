...
const customWorkingDirectory = path.join("/tmp", "custom_cwd");
...
tmr.registerMock("fs", {
  ...fs,
  existsSync: (p: string) => {
    if (p === customWorkingDirectory) return true;
    return fs.existsSync(p);
  },
  writeFileSync: () => {},
  copyFileSync: () => {},
  readFileSync: (p: string) => {
    if (p.includes("Repository_Name.txt")) return "myrepo";
    if (p.includes("Iteration_Id.txt")) return "1";
